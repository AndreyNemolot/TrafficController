...
//					t.stop();
	...
	private void restoreRules(Branch branch) {
		int total = 0;
		for(Iterator<Entry<Pair<Rule, VariableBinding>, Integer>> ruleAppIter = rulesApplied
				.entrySet().iterator(); ruleAppIter.hasNext(); ) {
			Entry<Pair<Rule, VariableBinding>, Integer> ruleBranchEntry = ruleAppIter.next();
		...
		for(Iterator<Entry<Fact, Integer>> iter = partialBindings.entrySet().iterator(); iter
				.hasNext(); ) {
			Entry<Fact, Integer> entry = iter.next();
