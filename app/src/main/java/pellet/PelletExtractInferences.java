...
public class PelletExtractInferences extends PelletCmdApp {
	...
	private void extractInferences() {
		...
		if( SetUtils.intersects( selector, StatementType.ALL_CLASS_STATEMENTS ) ) {
			startTask( "Extracting class statements" );
			extractor.extractClassModel( extracted );
			finishTask( "Extracting class statements" );
		}

		if( SetUtils.intersects( selector, StatementType.ALL_PROPERTY_STATEMENTS ) ) {
		...
		if( SetUtils.intersects( selector, StatementType.ALL_INDIVIDUAL_STATEMENTS ) ) {
