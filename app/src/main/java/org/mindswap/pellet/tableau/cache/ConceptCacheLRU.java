...
public class ConceptCacheLRU extends AbstractConceptCache implements ConceptCache {
	...
	public ConceptCacheLRU(KnowledgeBase kb, int maxSize) {
		...
		nonPrimitive = new LinkedHashMap<ATermAppl, CachedNode>( 16, 0.75f, true ) {
			protected boolean removeEldestEntry(Entry<ATermAppl, CachedNode> eldest) {
	...
	public Set<Entry<ATermAppl, CachedNode>> entrySet() {
		Set<Entry<ATermAppl, CachedNode>> returnSet;
		returnSet = new HashSet<Entry<ATermAppl, CachedNode>>( primitive.entrySet() );
	...
	public void putAll(Map<? extends ATermAppl, ? extends CachedNode> t) {
		for( Entry<? extends ATermAppl, ? extends CachedNode> entry : t.entrySet() ) {
